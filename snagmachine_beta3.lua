...
function getpkm(player,pkm) -- Get data for a given player's PKM as a table of bytes from [0,219]
	if player==0 then		-- Fetch offset of PKM of a given player
				start=Apkms[pkm]		-- offset of the PKM for player 1 (player0)
		elseif player==1 then
				start=Bpkms[pkm] 		-- offset of the PKM for player 2 (player1)
	end
	if pkm>6 then 			-- Error handle (Bad PKM req)
		print(sf("Error: Requested PKM %d -- Not supported.",pkm)) 
	end
	if player>1 then 		-- Error handle (Bad Player req)
		print(sf("Error: Requested Player %d -- Not supported.",player))
	end
...
function main()				-- Do work, son.
	...
	if enbl==0 then on=(on+1)%2 enbl=1  -- Enable and do the Data Parsing, clear any pre-existing snag data
			...
			if gi~=1 then								-- If Language is not supported (Only English is)
				print("This language ROM is not supported") 
			end
			if md(0x023A803C+g)==0x74726170 then		-- Get Battle Mode
						battlemode="PalPad Doubles"
						Apkmstart=0x023A8058+g
						Bpkmstart=0x023A85B8+g
				elseif md(0x02257B88+g)==0x74726170 then
						battlemode="Official Online Doubles"
						Apkmstart=0x02257BA4+g
						Bpkmstart=0x02258104+g
				elseif md(0x0224C7FC+g)==0x74726170 then
						battlemode="WiFi Tournament Doubles"
						Apkmstart=0x0224C818+g
						Bpkmstart=0x0224CD78+g
				else print() print("Currently not in a supported battle mode.") print() 
			end
			for i=1,6 do								-- Set up the offsets table for PKM reading.
				Apkms[i]=Apkmstart+0xDC*(i-1)
				Bpkms[i]=Bpkmstart+0xDC*(i-1)
			end
			print(sf("Battle Mode: %s",battlemode))		-- Print the Battle Mode
		...
		Bpkm1=getpkm(1,1) 
			...
			if Bspec1~="None" then snag=snag+1
			...
			Babilnum1=ability(Bpkm1[0x15])
		print(Bspec1.." ("..Bnature1..") @ "..Bitem1.."  #"..Babilnum1)
		...
		print("Stats: "..Bchp1.." | "..Bcat1.." | "..Bcde1.." | "..Bcsa1.." | "..Bcsd1.." | "..Bcsp1)	
			pidfield[snag]=Bpkm1[0x0]+Bpkm1[0x1]*0x100+Bpkm1[0x2]*0x10000+Bpkm1[0x3]*0x1000000
			tidfield[snag]=Bpkm1[0xC]+Bpkm1[0xD]*0x100
			sidfield[snag]=Bpkm1[0xE]+Bpkm1[0xF]*0x100
			dexfield[snag]=Bspec1
			datfield[snag]=string.format("")
			for i=0,135 do
				datfield[snag]=datfield[snag]..string.format("%02X",Bpkm1[i])
			end
		print() end
		Bpkm2=getpkm(1,2)
			Bspec2=species(Bpkm2[8]+0x100*Bpkm2[9])
			if Bspec2~="None" then snag=snag+1
			...
			Babilnum2=ability(Bpkm2[0x15])
		print(Bspec2.." ("..Bnature2..") @ "..Bitem2.."  #"..Babilnum2)
		...
		print("Stats: "..Bchp2.." | "..Bcat2.." | "..Bcde2.." | "..Bcsa2.." | "..Bcsd2.." | "..Bcsp2)	
			pidfield[snag]=Bpkm2[0x0]+Bpkm2[0x1]*0x100+Bpkm2[0x2]*0x10000+Bpkm2[0x3]*0x1000000
			tidfield[snag]=Bpkm2[0xC]+Bpkm2[0xD]*0x100
			sidfield[snag]=Bpkm2[0xE]+Bpkm2[0xF]*0x100
			dexfield[snag]=Bspec2
			datfield[snag]=string.format("")
			for i=0,135 do
				datfield[snag]=datfield[snag]..string.format("%02X",Bpkm2[i])
			end	
		print() end
		Bpkm3=getpkm(1,3)
			Bspec3=species(Bpkm3[8]+0x100*Bpkm3[9])
			if Bspec3~="None" then snag=snag+1
			...
			Babilnum3=ability(Bpkm3[0x15])
		print(Bspec3.." ("..Bnature3..") @ "..Bitem3.."  #"..Babilnum3)
		...
		print("Stats: "..Bchp3.." | "..Bcat3.." | "..Bcde3.." | "..Bcsa3.." | "..Bcsd3.." | "..Bcsp3)	
			pidfield[snag]=Bpkm3[0x0]+Bpkm3[0x1]*0x100+Bpkm3[0x2]*0x10000+Bpkm3[0x3]*0x1000000
			tidfield[snag]=Bpkm3[0xC]+Bpkm3[0xD]*0x100
			sidfield[snag]=Bpkm3[0xE]+Bpkm3[0xF]*0x100
			dexfield[snag]=Bspec3
			datfield[snag]=string.format("")
			for i=0,135 do
				datfield[snag]=datfield[snag]..string.format("%02X",Bpkm3[i])
			end
		print() end
		Bpkm4=getpkm(1,4)
			Bspec4=species(Bpkm4[8]+0x100*Bpkm4[9])
			if Bspec4~="None" then snag=snag+1
			...
			Babilnum4=ability(Bpkm4[0x15])
		print(Bspec4.." ("..Bnature4..") @ "..Bitem4.."  #"..Babilnum4)
		...
		print("Stats: "..Bchp4.." | "..Bcat4.." | "..Bcde4.." | "..Bcsa4.." | "..Bcsd4.." | "..Bcsp4)	
			pidfield[snag]=Bpkm4[0x0]+Bpkm4[0x1]*0x100+Bpkm4[0x2]*0x10000+Bpkm4[0x3]*0x1000000
			tidfield[snag]=Bpkm4[0xC]+Bpkm4[0xD]*0x100
			sidfield[snag]=Bpkm4[0xE]+Bpkm4[0xF]*0x100
			dexfield[snag]=Bspec4
			datfield[snag]=string.format("")
			for i=0,135 do
				datfield[snag]=datfield[snag]..string.format("%02X",Bpkm4[i])
			end
		print() end
		Bpkm5=getpkm(1,5)
			Bspec5=species(Bpkm5[8]+0x100*Bpkm5[9])
			if Bspec5~="None" then snag=snag+1
			...
			Babilnum5=ability(Bpkm5[0x15])
		print(Bspec5.." ("..Bnature5..") @ "..Bitem5.."  #"..Babilnum5)
		...
		pidfield[snag]=Bpkm5[0x0]+Bpkm5[0x1]*0x100+Bpkm5[0x2]*0x10000+Bpkm5[0x3]*0x1000000
		tidfield[snag]=Bpkm5[0xC]+Bpkm5[0xD]*0x100
		sidfield[snag]=Bpkm5[0xE]+Bpkm5[0xF]*0x100
		dexfield[snag]=Bspec5
		datfield[snag]=string.format("")
			for i=0,135 do
				datfield[snag]=datfield[snag]..string.format("%02X",Bpkm5[i])
			end
		print() end
		Bpkm6=getpkm(1,6)
			Bspec6=species(Bpkm6[8]+0x100*Bpkm6[9])
			if Bspec6~="None" then snag=snag+1
			...
			Babilnum6=ability(Bpkm6[0x15])
		print(Bspec6.." ("..Bnature6..") @ "..Bitem6.."  #"..Babilnum6)
		...
		print("Stats: "..Bchp6.." | "..Bcat6.." | "..Bcde6.." | "..Bcsa6.." | "..Bcsd6.." | "..Bcsp6)	
			pidfield[snag]=Bpkm6[0x0]+Bpkm6[0x1]*0x100+Bpkm6[0x2]*0x10000+Bpkm6[0x3]*0x1000000
			tidfield[snag]=Bpkm6[0xC]+Bpkm6[0xD]*0x100
			sidfield[snag]=Bpkm6[0xE]+Bpkm6[0xF]*0x100
			dexfield[snag]=Bspec6
			datfield[snag]=string.format("")
			for i=0,135 do
				datfield[snag]=datfield[snag]..string.format("%02X",Bpkm6[i])
			end
		print() end
		
		speedlist={}					-- Arrange a Speed Table; Blank out any pre-existing table with an empty table.
		sA1={spe=Acsp1,Aspec1,Acsp1,1} ti(speedlist, sA1)	-- Insert first PKM and successive PKMs if data is present. 
		if Aspec2~="None" then sA2={spe=Acsp2,Aspec2,Acsp2,1} ti(speedlist, sA2) end	-- Include Speed for sorting
		if Aspec3~="None" then sA3={spe=Acsp3,Aspec3,Acsp3,1} ti(speedlist, sA3) end	-- Include Species for display
		if Aspec4~="None" then sA4={spe=Acsp4,Aspec4,Acsp4,1} ti(speedlist, sA4) end	-- Include Speed count ^
		if Aspec5~="None" then sA5={spe=Acsp5,Aspec5,Acsp5,1} ti(speedlist, sA5) end	-- Include which player#.
		...
		ts(speedlist,function (v1, v2)	--Sort the Speed Table from Fastest->Slowest
             return v1.spe > v2.spe
           end)
		pkmcount=tl(speedlist)			--Count the # of Pokemon in the Battle
		snagemrdy=0 display=0			--Done with loading of PKM data. Ready secondary functions.
		end
    end
