...
io.write("HP Type,")
io.write("Power,")
io.write("PreShadow PID,")
io.write("Locked Nature\n")
										...
										gui.text(0,50 string.format("Spe: %d",testSpe))
											if nltotal>0 then
												...
												preshadowgend=preshadow%256														
													if (GenderCheck[1]==0 and preshadowgend<GenderVal[1]) or (GenderCheck[1]==1 and preshadowgend>= GenderVal[1]) or (GenderCheck[1]==2) or (GLOFF==0) then
														...
														if nltotal>1 then
															for cl=2,nltotal do
																...
																if errors==0 then
																		...
																		io.write(nextseed(originseed,2)..",")
																		io.write(testPID..",")
														...
														io.write(string.format("%8X", nextseed(originseed,2))..",")
														io.write(testPID..",")
														io.write(naturetable[testnature+1]..",")
														io.write(shinyPID..",")
														io.write(naturetable[shinyPID%25+1]..",")
											...
											preshadow=rshift(prevseed(originseed,1),16)*0x10000+rshift(originseed,16)
												currentseed=prevseed(originseed,5)
														...
														io.write(string.format("%8X", nextseed(originseed,2))..",")
														io.write(string.format("%8X", testPID)..",")
														io.write(naturetable[testnature+1]..",")
														io.write(string.format("%8X", shinyPID)..",")
														io.write(naturetable[shinyPID%25+1]..",")
														io.write(testHP..",")
														io.write(testAtk..",")
														io.write(testDef..",")
